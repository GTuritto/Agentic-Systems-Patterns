...
```mermaid
flowchart LR
  G[Goal and budget] --> O[Observe state]
  O --> D[Model proposes next step]
  D --> V[Validate action]
  V -->|allowed| A[Act]
  V -->|denied| X[Refuse or escalate]
  A --> R[Record observation]
  R --> S{Stop condition?}
  S -->|continue| O
  S -->|done| F[Return typed result]
  S -->|limit reached| X
```
...
- [Structured Output](../structured-output-pattern/README.md)
